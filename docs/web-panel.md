...
- `Hardware`: модель устройства
- `Firmware`: версия прошивки с датой сборки
- `Public Key` (Публичный ключ)

## Обновление прошивки (OTA)

Этот раздел позволяет безопасно загружать прошивку на устройство по Wi-Fi.

Чтобы перепрошить устройство, нажмите на поле выбора файла и выберите файл прошивки с расширением `.bin`, при этом кнопка `Upload` активируется.
Нажмите кнопку `Upload` и после подтверждения начнётся загрузка прошивки с индикацией в процентах.

После успешного завершения устройство автоматически перезагрузится с новой прошивкой.
...
- `Trends`: заряд батареи, свободная память, пакетная активность (RX/TX в виде составного графика), частота ошибок приёма (RX error rate), сигнал, уровень шума, загрузка ядра процессора, температура MCU (с цветовой индикацией теплового порога) и, если включён GPS, спутники; карточки графиков показывают диапазон `min`/`max`, подсветка при наведении синхронизирована по всем графикам
...
5. частота ошибок приёма
6. загрузка ядра процессора
7. сигнал
8. спутники, когда GPS включён
...
### Дистанционное обновление прошивки (OTA)

1. Откройте панель
2. Найдите `Firmware Update` и нажмите на поле выбора файла
3. Выберите файл с прошивкой `.bin`
4. Нажмите `Upload` и подтвердите действие
5. Дождитесь загрузки прошивки и перезагрузки устройства
